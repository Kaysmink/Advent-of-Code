...
numbers = [re.findall(r'\d|one|two|three|four|five|six|seven|eight|nine', line, overlapped=True) for line in Input]
numbers = [[number if number.isnumeric() else str(numberdict[number]) for number in line] for line in numbers]
